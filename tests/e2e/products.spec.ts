...
import { getFilteredProducts, getTotalPriceOfProducts, getExpectedCount } from '../../business-objects/ProductsApiBO';
...
test.describe('Products - Add to Basket', () => {
    ...
    test('Complete Purchase Flow - Successful Checkout 1 item', async ({ page, request, testData }) => {
        const products = (await getFilteredProducts(request, {
    ...
    test('Complete Purchase Flow - Successful Checkout - last item', async ({ page, request, testData }, testInfo) => {
        const products = (await getFilteredProducts(request, {
            quantityFilter: q => q.quantity === 1
        })).slice(0, 1);
