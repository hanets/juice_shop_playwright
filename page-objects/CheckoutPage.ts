...
export class CheckoutPage {
    ...
    constructor(page: Page) {
        ...
        this.proceedToPaymentButton = page.getByRole('button', { name: 'Proceed to payment selection' });
        
        // Delivery method
        this.deliveryPageHeading = page.getByRole('heading', { name: 'Choose a delivery speed' });
        this.standardDeliveryRadio = page.getByRole('row', { name: 'Standard Delivery 0.00¤ 5 Days' }).locator('input[type="radio"]');
        ...
        // Order confirmation
        this.orderConfirmationHeading = page.getByRole('heading', { name: 'Thank you for your purchase!' });
    ...
    async addDeliveryAddress(addressData: {
        ...
        await expect(this.page.getByText(`The address at ${addressData.city} has been successfully added to your addresses.`)).toBeVisible();
        await expect(this.page.getByText(addressData.name)).toBeVisible();
        await expect(this.page.getByText(`${addressData.address}, ${addressData.city}, ${addressData.state}, ${addressData.zipCode}`)).toBeVisible();
    }
    ...
    async verifyOrderReview(userEmail: string, expectedTotal: string, paymentMethodDigits?: string): Promise<void> {
        await expect(this.page.getByRole('heading', { name: `Your Basket (${userEmail})` })).toBeVisible();
        ...
        await expect(this.page.locator('tr', { hasText: 'Total Price' }).getByText(expectedTotal)).toBeVisible();
    }
    ...
    async verifyOrderConfirmation(productNames: string[]): Promise<void> {
        ...
        await expect(this.page.getByText('Your order has been placed and is being processed.')).toBeVisible();
    ...
    async trackOrder(orderId: string, productNames: string[]): Promise<void> {
        ...
        await expect(this.page.getByRole('heading', { name: `Search Results - ${orderId}` })).toBeVisible();
